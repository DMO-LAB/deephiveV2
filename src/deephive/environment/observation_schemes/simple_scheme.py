...
class SimpleObservationScheme(ObservationScheme):
    
    def generate_observation(self, pbest, use_gbest=False, std_type="euclidean", 
                             ratio=0.5, include_gbest=False, include_lbest=False):
        ...
        for agent in range(self.env.n_agents):
            ...
            for dim in range(self.env.n_dim):
                obs_values = self._get_obs_for_dim(agent, agent_nb, dim, pbest, gbest, 
                                                   use_gbest, include_gbest, include_lbest)
    ...
    def _get_obs_for_dim(self, agent, agent_nb, dim, pbest, gbest, use_gbest, 
                         include_gbest=False, include_lbest=False):
        ...
        if include_lbest:
            obs.append(self.env.state[agent][dim]- self.env.lbest_positions[agent][dim])
        
        if include_gbest:
            obs.append(self.env.state[agent][dim] - gbest[dim])
        if use_gbest: 
            # replace the third and fourth elements with the distance to gbest
            obs[1] = self.env.state[agent][dim] - gbest[dim]
        return obs
