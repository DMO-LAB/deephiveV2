...
def f21(x, rotations=None, shifts=None, negative=True):
    """
    ...
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2100 if \
                not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2100


def f22(x, rotations=None, shifts=None, negative=True):
    """
    ...
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2200 if \
        not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2200


def f23(x, rotations=None, shifts=None, negative=True):
    """
    ...
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2300 if \
        not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2300


def f24(x, rotations=None, shifts=None, negative=True):
    """
    ...
    biases = np.array([0.0, 100.0, 200.0, 300.0])
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2400 if \
        not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2400


def f25(x, rotations=None, shifts=None, negative=True):
    """
    ...
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2500 if \
        not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2500


def f26(x, rotations=None, shifts=None, negative=True):
    """
    ...
    biases = np.array([0.0, 100.0, 200.0, 300.0, 400.0])
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2600 if \
        not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2600


def f27(x, rotations=None, shifts=None, negative=True):
    """
    ...
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2700 if \
        not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2700


def f28(x, rotations=None, shifts=None, negative=True):
    """
    ...
    biases = np.array([0.0, 100.0, 200.0, 300.0, 400.0, 500.0])
    return _composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) + 2800 if \
        not negative else -_composition(x, rotations, shifts, funcs, sigmas, lambdas, biases) - 2800


def f29(x, rotations=None, shifts=None, shuffles=None, negative=True):
    """
    ...
    return _compose_hybrids(x, rotations, shifts, shuffles, funcs, sigmas, offsets, biases) + 2900 \
        if negative else -_compose_hybrids(x, rotations, shifts, shuffles, funcs, sigmas, offsets, biases) - 2900



def f30(x, rotations=None, shifts=None, shuffles=None, negative=True):
    """
    ...
    return _compose_hybrids(x, rotations, shifts, shuffles, funcs, sigmas, offsets, biases) + 3000 \
        if negative else -_compose_hybrids(x, rotations, shifts, shuffles, funcs, sigmas, offsets, biases) - 3000
