...
class FullRewardScheme2(RewardScheme):
    ...
    def compute_reward(self):
        ...
        reward = np.divide(self.env.state[:, -1] - self.env.prev_state[:, -1], 1 - self.env.prev_state[:, -1])
        # clip reward if it is > 1 or < -1 to 1 or -1 respectively in order to avoid exploding gradients
    ...
    def _post_process_rewards(self, reward):
        ...
        # ensure there is no nan reward or extremely large reward
        reward[np.isnan(reward)] = 1
        reward[reward > 1] += 1
